  ...
  const options = {
    ...
    scales: {
      x: {
        ticks: {
          font: {
            size: window.innerWidth < 480 ? 10 : 12, // Adjust font size for mobile screens
          },
        },
      },
      y: {
    ...
    plugins: {
      legend: {
        display: true,
        position: window.innerWidth < 480 ? "top" : "right", // Change legend position on mobile
        labels: {
          boxWidth: window.innerWidth < 480 ? 10 : 20, // Smaller legend boxes for small screens
        },
      },
    },
  };
        ...
        const fetchHistoricalAQI = async () => {
          try {
            const response = await fetch(
              `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${lat}&longitude=${log}&hourly=pm10,pm2_5,carbon_monoxide,nitrogen_dioxide,sulphur_dioxide,ozone`
            );
